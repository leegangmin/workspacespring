...
import com.poseidon.service.MessageServiceImpl;
...
public class MessageController {
	...
	public ModelAndView message(CommandMap map,  HttpServletRequest request) {
		...
		if(session.getAttribute("sm_id") != null) {
			...
			if(map.containsKey("openmsg")) {
				//읽음 처리를 먼저 해주세요.
				messageService.readMessage(map.getMap());//sm_id, openmsg
				//사용자가 메시지를 읽기를 원함
				Map<String, Object> detail = 
						messageService.messageDetail(map.getMap());//sm_id, openmsg
	...
	public String sendMessage(CommandMap commandMap, HttpServletRequest request) {
		HttpSession session = request.getSession();
		if(session.getAttribute("sm_id") != null) {//로그인 함
			//로그인 사용자 id = 보내는 사람
			commandMap.put("sm_id", session.getAttribute("sm_id"));
